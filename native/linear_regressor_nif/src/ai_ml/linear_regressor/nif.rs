...
pub fn fit_little_rayon<'a>(env: Env<'a>, args: &[Term<'a>])-> NifResult<Term<'a>> {
  let pid = env.pid();
  let mut my_env = OwnedEnv::new();
  ...
  std::thread::spawn(move ||  {
    my_env.send_and_clear(&pid, |env| {
      let result: NifResult<Term> = (|| {
        ...
        let (ans, fit_ot) = measure!({
          lr::fit_little_rayon(&tx, &ty, alpha, iterations)
        });
        fit_time = fit_time + fit_ot;
        println!("fit_time:{:?}", fit_time);
        // println!("theta:{:?}", ans);
...
pub fn fit_filled_rayon<'a>(env: Env<'a>, args: &[Term<'a>])-> NifResult<Term<'a>> {
  let pid = env.pid();
  let mut my_env = OwnedEnv::new();
  ...
  std::thread::spawn(move ||  {
    my_env.send_and_clear(&pid, |env| {
      let result: NifResult<Term> = (|| {
        ...
        let (ans, fit_ot) = measure!({
          lr::fit_filled_rayon(&tx, &ty, alpha, iterations)
        });
        fit_time = fit_time + fit_ot;
        println!("fit_time:{:?}", fit_time);
        // println!("theta:{:?}", ans);
...
pub fn benchmarks_fit_little_rayon<'a>(env: Env<'a>, args: &[Term<'a>])-> NifResult<Term<'a>> {
  let pid = env.pid();
  let mut my_env = OwnedEnv::new();
      ...
      let result: NifResult<Term> = (|| {
        ...
        (1..=num as usize).collect::<Vec<_>>().iter().for_each(|&n| {
          let minsec = (1..=10).collect::<Vec<_>>().iter().map(|_|
            match benchmark_fit_little_rayon(n, &x, &y, alpha, iterations) {
              Ok(realsec) => realsec,
...
pub fn benchmarks_fit_filled_rayon<'a>(env: Env<'a>, args: &[Term<'a>])-> NifResult<Term<'a>> {
  let pid = env.pid();
  let mut my_env = OwnedEnv::new();
  ...
  std::thread::spawn(move ||  {
    my_env.send_and_clear(&pid, |env| {
      let result: NifResult<Term> = (|| {
        ...
        (1..=num as usize).collect::<Vec<_>>().iter().for_each(|&n| {
          let minsec = (1..=10).collect::<Vec<_>>().iter().map(|_|
            match benchmark_fit_filled_rayon(n, &x, &y, alpha, iterations) {
              Ok(realsec) => realsec,
              Err(e) => {println!("error: {}", e); ::std::f64::NAN},
            }
          ).fold(0.0/0.0, |m, v| v.min(m));
...
pub fn benchmark_fit_filled_rayon(
  n: usize,
  x: &Vec<Vec<f64>>, 
  y: &Vec<Vec<f64>>, 
  alpha: f64,
  iterations: i64) -> Result<f64, ThreadPoolBuildError> {
    {
        match set_num_threads(n) {
          Ok(pool) => pool.install(|| {
            ...
            lr::fit_filled_rayon(x, y, alpha, iterations);
...
pub fn benchmark_fit_little_rayon(
  n: usize,
  x: &Vec<Vec<f64>>, 
  y: &Vec<Vec<f64>>, 
  alpha: f64,
  iterations: i64) -> Result<f64, ThreadPoolBuildError> {
    {
        match set_num_threads(n) {
          Ok(pool) => pool.install(|| {
            let start_time = time::get_time();
            lr::fit_little_rayon(x, y, alpha, iterations);
            let end_time = time::get_time();
            let diffsec = end_time.sec - start_time.sec;   // i64
            let diffsub = end_time.nsec - start_time.nsec; // i32
            let realsec = diffsec as f64 + diffsub as f64 * 1e-9;
